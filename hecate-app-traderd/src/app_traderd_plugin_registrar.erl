...
%%% @end
-module(app_traderd_plugin_registrar).
...
init([]) ->
    ...
    case attempt_registration() of
        ok ->
            logger:info("[app_traderd_plugin_registrar] Registered with hecate-daemon");
        {error, Reason} ->
            logger:warning("[app_traderd_plugin_registrar] Could not register "
...
attempt_registration() ->
    ...
    case filelib:is_file(DaemonSocket) of
        ...
        true ->
            logger:info("[app_traderd_plugin_registrar] Found daemon socket at ~s, "
