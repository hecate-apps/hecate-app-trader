%%% @doc Path utilities for hecate-app-traderd.
%%%
%%% Provides purpose-specific path functions for the namespaced
%%% directory layout under ~/.hecate/hecate-app-traderd/:
%%%
...
%%% {hecate_app_traderd, [{data_dir, Path}]}.
%%% Default: ~/.hecate/hecate-app-traderd
%%% @end
-module(app_traderd_paths).
...
base_dir() ->
    case application:get_env(hecate_app_traderd, data_dir) of
        {ok, Dir} -> expand_path(Dir);
        undefined -> expand_path("~/.hecate/hecate-app-traderd")
    end.
